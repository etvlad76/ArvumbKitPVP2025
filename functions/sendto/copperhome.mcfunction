execute as @a[r=10, tag=!mayor, hasitem={item=tuff}] at @s if block ~ 147 ~ waxed_copper run tag @s add tolobby
execute as @a[r=10, tag=!mayor, hasitem={item=tuff}] at @s if block ~ 147 ~ air run tag @s remove tolobby
execute as @a[r=10, tag=!mayor, hasitem={item=tuff}] at @s if block ~ 147 ~ air run effect @s clear
execute as @a[r=10, tag=!mayor, hasitem={item=tuff}] at @s if block ~ 147 ~ air run tag @s remove timer
execute as @a[r=10, tag=!mayor, hasitem={item=tuff}] at @s if block ~ 147 ~ air run scoreboard players reset @s tick
